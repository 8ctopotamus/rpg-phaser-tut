...
RPG.Enemy = function(state, x, y, key, data) {
	...
	//'= +val' makes converts properties to numbers
	// because the values loaded from tile are text
	this.data.attack = +this.data.attack
	this.data.defense = +this.data.defense
	this.data.health = +this.data.health

	// add enegry health
	this.healthBar = new Phaser.Sprite(this.state.game, this.x, this.y, 'bar')
